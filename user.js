...
exports.dashboard = function(req, res){
  ...
  // console.log('current id ='+userId);
   ...
   var sql=`SELECT ServiceName,ServiceDescription,Price from Services;`;          
   db.query(sql, function(err, results){  
         req.session.user = results[0];
        /*console.log(results[1].ServiceName);
         console.log(results[1].ServiceDescription);
...
exports.profile = function(req, res){
   ...
   var sql=`SELECT Accounts.AccountID,Abonents.Surname,Abonents.FirstName,Contracts.MobileNumber,Contracts.Tarif FROM Accounts  JOIN Abonents on (Accounts.AccountID=Abonents.AbonentID) JOIN Contracts on (Accounts.AccountID=Contracts.AccountID)WHERE Accounts.AccountID=${userId};`;          
   db.query(sql, function(err, results){  
      if(err){
         console.log(err);
       };
...
exports.editprofile=function(req,res){
   ...
   db.query(` UPDATE Accounts SET pswrd=MD5('${pass}') where AccountID=${userId} `);

   db.query(`UPDATE Contracts SET Tarif="${tariff}" where AccountID=${userId};`, function(err, results){
   message='Confirmed'; 
   } );
   ...
   db.query(sql, function(err, results){  
         req.session.user = results[0];
         res.render('edit_profile.ejs',{user:results});
   });
